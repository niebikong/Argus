...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Argus stage-1 training for CSV + DeepResNet")
    ...
    parser.add_argument("--switch_epoch", type=int, default=130)
...
def main() -> None:
    ...
    base_dir = save_root / f"{args.noise_type}_{int(args.noise*100)}_Stage_1"
    save_dir = Path(args.load_dir).resolve() if args.load_dir else base_dir
    ensure_dir(save_dir)
    ...
    for epoch in range(epoch_resume, args.max_epochs):
        ...
        do_eval = ((args.eval_interval > 0 and (epoch + 1) % args.eval_interval == 0) or epoch == args.switch_epoch - 1 or epoch == args.max_epochs - 1)
        do_save = (epoch == args.max_epochs - 1)
        do_plot = ((args.plot_interval > 0 and (epoch + 1) % args.plot_interval == 0) or epoch == args.max_epochs - 1)
        ...
        net.train()
        for i, (x, labels, index) in enumerate(trainloader):
            ...
            s_neg = torch.log(torch.clamp(1.0 - F.softmax(logits.float(), dim=-1), min=1e-5, max=1.0))
            ...
            if epoch >= args.switch_epoch:
                if epoch == args.switch_epoch and i == 0:
                    logger.info("Switch to SelNL")
                avg_hist = train_preds_hist.mean(1)
                conf = avg_hist[index_cpu, labels.detach().cpu()] < (1.0 / float(num_classes))
                labels_neg[conf.to(device)] = -100
                labels = labels * 0 - 100
            else:
                labels = labels * 0 - 100
        ...
        if do_eval:
            ...
            logger.info("\tTESTING...skipped at epoch %d (eval_interval=%d)", epoch, args.eval_interval)
        ...
        test_acc_for_best = test_metrics["acc"] if do_eval else float("-inf")
        is_best = do_eval and (test_acc_for_best > best_test_acc)
        if do_eval:
            best_test_acc = max(test_acc_for_best, best_test_acc)
        ...
        state = {
            ...
            "test_metrics": (test_metrics if do_eval else None),
